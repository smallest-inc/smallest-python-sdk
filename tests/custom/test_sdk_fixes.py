...
def test_call_analytics_rename_and_alias():
    from smallestai.agents.helpers import Call, CallAnalytics
...
def test_helpers_import_clean():
    import smallestai.agents.helpers  # noqa: F401  (#17 — requests dep present)


def test_as_page_normalizes_shapes():
    from smallestai.agents.helpers import as_page  # #25
...
def test_require_id_guard():
    from smallestai.agents.helpers import require_id  # #18
    for bad in ["", "   ", None]:
        with pytest.raises(ValueError):
            require_id(bad)
