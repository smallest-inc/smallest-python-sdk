...
def agent_id_helper():
    r = c.agents.agents.create_a_new_agent(name="triage-base-2026-06-18")
...
try:
    ...
    c2.agents.user.get_user_details()
...
try:
    r = c.agents.agents.create_a_new_agent(name="triage-fm-2026-06-18", first_message="Banana phone")
    aid = getattr(r, "data", None)
    g = c.agents.agents.get_agent_by_id(id=aid)
...
try:
    methods = [m for m in dir(c.agents.calls) if not m.startswith("_")]
...
# --- #18 get_agent_by_id("") ---
try:
    g = c.agents.agents.get_agent_by_id(id="")
    d = getattr(g, "data", None)
...
try:
    c.agents.agents.get_agent_by_id(id="nonexistent")
...
try:
    a = getattr(c.agents.agents.create_a_new_agent(name="triage-dup-2026-06-18"), "data", None)
    try:
        b = getattr(c.agents.agents.create_a_new_agent(name="triage-dup-2026-06-18"), "data", None)
...
rec("#22", "BUG" if hasattr(c.agents.agents, "update_workflow_configuration") else "FIXED",
    "sdk", "update_workflow_configuration on public surface" if hasattr(c.agents.agents, "update_workflow_configuration") else "hidden")

# --- #24 pydantic serialization warning on list ---
try:
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        c.agents.agents.get_all_agents()
...
try:
    cr = c.agents.agents.create_a_new_agent(name="triage-shape-2026-06-18")
    create_t = type(getattr(cr, "data", None)).__name__
    gt = type(getattr(c.agents.agents.get_agent_by_id(id=getattr(cr, "data", None)), "data", None)).__name__
...
try:
    import smallestai.agents.helpers as h
