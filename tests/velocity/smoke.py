...
print("== user ==")
try:
    u = c.agents.user.get_user_details()
    print("user ok:", getattr(u, "data", u))
except Exception as e:
    print("user FAILED:", type(e).__name__, str(e)[:200])
    print("user namespace methods:", [m for m in dir(c.agents.user) if not m.startswith("_")])

print("== create agent with first_message ==")
r = c.agents.agents.create_agent(name="sdk-smoke-2026-06-18", first_message="Banana phone greeting")
aid = getattr(r, "data", None)
print("created agent id:", aid)

print("== get agent back -> firstMessage ==")
g = c.agents.agents.get_agent(id=aid)
...
print("== list agents ==")
lst = c.agents.agents.list_agents()
