...
from smallestai.agents.helpers import as_page, require_id
...
def _create():
    aid = c.agents.agents.create_agent(name="sdk-itest-agent", first_message="Hi from itest").data
...
def _get():
    aid = made_agents[-1]
    a = c.agents.agents.get_agent(id=aid).data
...
def _list():
    pg = as_page(c.agents.agents.list_agents())
    return f"{len(pg.items)} agents (total={pg.total_count})"


def _update():
    aid = made_agents[-1]
    c.agents.agents.update_agent(id=aid, description="updated by itest")
    return "updated"


def _duplicate():
    aid = made_agents[-1]
    org = c.agents.user.get_user_details().data.organization_id
    dup = c.agents.agents.duplicate_agent(id=aid, target_organization_id=org)
...
def _templates():
    pg = as_page(c.agents.agent_templates.list_agent_templates())
    return f"{len(pg.items)} templates"


def _kb_list():
    return f"{len(as_page(c.agents.knowledge_base.list()).items)} KBs"


def _phone_list():
    pg = as_page(c.agents.phone_numbers.list())
    return f"{len(pg.items)} numbers"


def _calls_list():
    pg = as_page(c.agents.calls.list())
...
def _bad_id():
    try:
        c.agents.agents.get_agent(id="nonexistent")
...
check("user.get_user_details", lambda: c.agents.user.get_user_details().data.user_email)
