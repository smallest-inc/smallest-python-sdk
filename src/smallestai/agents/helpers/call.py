...
Usage:
    from smallestai.agents.helpers import Call
...
class CallAnalytics:
    ...
    call use ``client.agents.calls.start_an_outbound_call(...)``.
...
class Call(CallAnalytics):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        warnings.warn(
            "smallestai.agents.helpers.Call is deprecated and will be removed in a "
            "future major version. Use CallAnalytics instead (it is analytics-only "
            "and does not place calls; to start a call use "
            "client.agents.calls.start_an_outbound_call).",
            DeprecationWarning,
            stacklevel=2,
        )
