...
from smallestai.agents.crew.clients.base import BaseLLMClient
from smallestai.agents.crew.clients.types import ChatChunk, ChatResponse, ToolCall
from smallestai.agents.crew.type import NOT_GIVEN, NotGivenOr, is_given
...
class OpenAIClient(BaseLLMClient):
    ...
    def electron(
        ...
        **kwargs: Any,
        ...
        """
        Create a client for Smallest's Electron LLM (OpenAI-compatible).

        Reads the key from ``SMALLEST_API_KEY`` when ``api_key`` is not given,
        and points at the Smallest Waves base URL so callers don't hard-code it.

        Example:
            llm = OpenAIClient.electron()
        """
        return cls(
            model=model,
    ...
    def _create_client(
        ...
        if not api_key:
            raise ValueError(
                "OpenAIClient requires an api_key. Pass it explicitly or set "
                "OPENAI_API_KEY in your environment. For Smallest's Electron model, "
                "use OpenAIClient.electron(api_key=os.getenv('SMALLEST_API_KEY'))."
            )
