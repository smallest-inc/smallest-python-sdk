...
from smallestai.agents.crew.nodes import CrewNode
from smallestai.agents.crew.task_manager import TaskManager, TaskManagerParams
...
class SessionConfig(BaseModel):
    ...
    edges: List[Tuple[CrewNode, CrewNode]] = []
    nodes: List[CrewNode] = []
...
class SinkNode(CrewNode):
    ...
    def __init__(self, session: "CrewSession", name: str = "Sink"):
        super().__init__(name=name)
...
class CrewSession:
    ...
    def __init__(
        ...
        self.nodes: List[CrewNode] = []
        self.edges: List[Tuple[CrewNode, CrewNode]] = []
    ...
    def __repr__(self):
        return f"<CrewSession: {self.name} ({len(self.nodes)} nodes)>"
