# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ....core.pydantic_utilities import IS_PYDANTIC_V2
from ....core.unchecked_base_model import UncheckedBaseModel
from .response_function_call_arguments_done_type import ResponseFunctionCallArgumentsDoneType


class ResponseFunctionCallArgumentsDone(UncheckedBaseModel):
    type: typing.Optional[ResponseFunctionCallArgumentsDoneType] = None
    event_id: typing.Optional[str] = None
    response_id: typing.Optional[str] = None
    call_id: typing.Optional[str] = None
    name: typing.Optional[str] = None
    arguments: typing.Optional[str] = pydantic.Field(default=None)
    """
    Full arguments JSON. Parse with `JSON.parse` / `json.loads` before passing to your tool.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
