...
from .types.tts_request_message import TtsRequestMessage
from .types.tts_response_message import TtsResponseMessage
...
TtsSocketClientResponse = typing.Union[TtsResponseMessage]
...
class AsyncTtsSocketClient(EventEmitterMixin):
    ...
    async def __aiter__(self):
        async for message in self._websocket:
            if isinstance(message, bytes):
                ...
                try:
                    yield construct_type(type_=TtsSocketClientResponse, object_=json.loads(message))  # type: ignore
                except Exception:
                    _logger.warning(
        ...
        try:
            async for raw_message in self._websocket:
                if isinstance(raw_message, bytes):
                    ...
                    try:
                        parsed = construct_type(type_=TtsSocketClientResponse, object_=json_data)  # type: ignore
                    except Exception:
                        _logger.warning(
                            "Skipping unknown WebSocket message; update your SDK version to support new message types."
                        )
    ...
    async def synthesize_streaming_tts(self, message: TtsRequestMessage) -> None:
        """
        Send a message to the websocket connection.
        The message will be sent as a TtsRequestMessage.
        """
        await self._send_model(message)

    async def recv(self) -> TtsSocketClientResponse:
        """
        ...
        try:
            return construct_type(type_=TtsSocketClientResponse, object_=json_data)  # type: ignore
        except Exception:
...
class TtsSocketClient(EventEmitterMixin):
    ...
    def synthesize_streaming_tts(self, message: TtsRequestMessage) -> None:
        """
        Send a message to the websocket connection.
        The message will be sent as a TtsRequestMessage.
        """
    ...
    def recv(self) -> TtsSocketClientResponse:
        """
        Receive a message from the websocket connection.
        """
