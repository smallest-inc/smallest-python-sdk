...
class SessionConfig(UncheckedBaseModel):
    ...
    - `?model=hydra-v1.1`, ten voices: `zoe`, `maya`, `elena`, `ivy`, `grace`, `alex`, `aria`, `leo`, `sam`, `kai`.
    - `?model=hydra` currently routes to `hydra-v1.0`. This parameter will be deprecated in the future; pass an explicit version.
